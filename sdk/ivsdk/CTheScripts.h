...
public:
	...
	{
		// mingw port: the game's lookup takes the hash in esi and returns the
		// function pointer in eax (was MSVC __asm + __try/__except upstream)
		uint32_t funcPtr = AddressSetter::Get(0x1A76D0, 0x226AF0);
		uint32_t nativePtr;
		__asm__ volatile("call *%1" : "=&a"(nativePtr) : "r"(funcPtr), "S"(nativeHash) : "ecx", "edx", "memory", "cc");
		return nativePtr;
	}
